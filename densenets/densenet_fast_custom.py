...
from BatchNormDNNLayer_custom import *
# TODO: purge
...
def build_densenet(input_shape=(None, 3, 32, 32), input_var=None, classes=10,
                   ...
                   dropout=0, nonlinearity_name='relu'):
    """
...
def dense_block(network, num_layers, growth_rate, dropout, name_prefix, nonlinearity=rectify):
    # concatenated 3x3 convolutions
    for n in range(num_layers):
        conv = affine_relu_conv(network, channels=growth_rate,
                                filter_size=3, dropout=dropout,
                                name_prefix=name_prefix + '_l%02d' % (n + 1), nonlinearity=nonlinearity)
...
def transition(network, dropout, name_prefix, nonlinearity=rectify):
    ...
    network = affine_relu_conv(network, channels=network.output_shape[1],
                               ...
                               name_prefix=name_prefix, nonlinearity=nonlinearity)
...
def affine_relu_conv(network, channels, filter_size, dropout, name_prefix, nonlinearity=rectify):
